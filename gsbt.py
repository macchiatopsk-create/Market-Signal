...
def build_trades():
    ...
    v = C.norm(yf.Ticker("^VIX").history(period="5y")[["Open", "Close"]].dropna())
    ch = (v["Open"] / v["Close"].shift(1) - 1) * 100
    vm = {str(pd.Timestamp(k).date()): float(x) for k, x in ch.dropna().items()}
    try:
        x = C.norm(yf.Ticker("^VXN").history(period="5y")[["Open"]].dropna())
    ...
    dd = C.norm(yf.download("QQQ", period="5y", interval="1d",
