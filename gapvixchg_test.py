...
def main():
    ...
    chg=(v/v.shift(1)-1)*100
    prev={str(pd.Timestamp(d).date()):float(x) for d,x in chg.shift(1).dropna().items()}  # 전날 변화
    same={str(pd.Timestamp(d).date()):float(x) for d,x in chg.dropna().items()}           # 당일 변화
    out=[f"^VIX 일간변화율 맵 {len(prev)}일",
         "조건: 갭 & 첫1시간 커버>=30% · 진입 첫봉종가 · 타깃 전날종가 · 손절 당일극점 · 컷14:30"]
    BINS=[(-99,-5,"≤-5%"),(-5,-2,"-5~-2%"),(-2,2,"-2~+2%"),(2,5,"+2~5%"),
          (5,8,"+5~8%"),(8,99,"≥+8%")]
    for tk in ("QQQ","SPY"):
        rows=build(tk)
        out.append(f"\n{'='*134}\n[{tk}] 조건충족 {len(rows)}일\n{'='*134}")
        for sgn,nm in ((1,"갭업→숏"),(-1,"갭다운→롱")):
            ...
            rep(ss,"전체",out)
            out.append("   [전날 VIX 변화율 · 실전 사용 가능]")
            for lo,hi,lab in BINS:
                rep([r for r in ss if lo<=prev.get(r["d"],0)<hi],lab,out)
            out.append("   [당일 VIX 변화율 · 참고용(동시점)]")
            for lo,hi,lab in BINS:
...
if __name__=="__main__":
    ...
    json.dump({"at":dt.datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC"),"report":txt},
              open("gapvixchg_result.json","w"),ensure_ascii=False,indent=1)
